  ...
  return (
    <div>
      ...
      <p className="mt-2 max-w-2xl text-muted-foreground">
        Everything you need to go from <code className="rounded bg-muted px-1.5 py-0.5 text-sm">git clone</code> to
        production. The same guides live in the repo&apos;s{" "}
        <a
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2">
        {DOCS.map((d) => (
          <Link
            key={d.slug}
            href={`/docs/${d.slug}`}
            className="group rounded-2xl border border-border bg-background p-5 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-soft"
          >
