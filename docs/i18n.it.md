...
`source_commit` è la revisione del file inglese da cui la traduzione è stata scritta. Impostalo quando traduci, spostalo quando aggiorni:
...
Esce con un codice diverso da zero quando il file inglese è cambiato dopo la revisione registrata da una traduzione. **Questo è il controllo che vale la pena tenere.** Una traduzione vecchia non si rompe mai: continua a rendere, continua a sembrare finita, e diventa in silenzio le istruzioni di una versione precedente del tuo prodotto. L'unico modo per accorgersene leggendo è aprire i due file e confrontarli, quindi non lo fa nessuno.
